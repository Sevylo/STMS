                    ...
                    <div class="relative bg-white rounded-xl shadow-2xl max-w-lg w-full p-6 transform transition-all">
                        ...
                        <form method="POST" action="{{ route('tasks.store') }}">
                            ...
                            <div class="space-y-4">
                                ...
                                <div>
                                    <label class="block text-sm font-medium text-gray-700">Custom Reminder (Optional)</label>
                                    <input type="datetime-local" name="reminder_at" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                                    <p class="text-xs text-gray-500 mt-1">Leave blank for default reminders (7, 3, 1 days before deadline).</p>
                                </div>
                        ...
                        <div class="p-6">
                            ...
                            <div class="space-y-1 mb-4">
                                <div class="flex items-center text-xs text-gray-500 gap-2">
                                    ...
                                    <span>Due: {{ \Carbon\Carbon::parse($task->deadline)->format('M d, Y h:i A') }}</span>
                                </div>
                                @if($task->reminder_at)
                                <div class="flex items-center text-xs text-indigo-500 gap-2">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9"/></svg>
                                    <span>Reminder: {{ \Carbon\Carbon::parse($task->reminder_at)->format('M d, Y h:i A') }}</span>
                                </div>
                                @endif
                            </div>
